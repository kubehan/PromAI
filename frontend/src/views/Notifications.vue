<template>
  <div class="page-container">
    ...
    <el-dialog v-model="dialogVisible" :title="editingId ? '编辑通知渠道' : '新增通知渠道'" width="600" :close-on-click-modal="false">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="渠道类型" prop="channel_type">
          <el-select v-model="form.channel_type" style="width: 100%" :disabled="!!editingId">
        ...
        <el-form-item label="配置" prop="config_json">
          <el-input v-model="form.config_json" type="textarea" :rows="8" placeholder="请输入 JSON 配置..." />
        </el-form-item>
      </el-form>
      <template #footer>
...
function openCreate() { editingId.value = null; form.value = { channel_type: 'wechat_work', name: '', enabled: true, config_json: '{}' }; dialogVisible.value = true }
function openEdit(row: NotificationChannel) { editingId.value = row.id!; form.value = { ...row }; dialogVisible.value = true }
